...
commandWithOpenOptions('codegen [url]', 'open page and generate code for user actions',
    [
      ...
      ['--ui-host <host>', 'Host to serve recorder UI on; specifying this option opens UI in a browser tab'],
      ['--ui-port <port>', 'Port to serve recorder UI on, 0 for any free port; specifying this option opens UI in a browser tab'],
...
async function codegen(options: Options & { target: string, output?: string, testIdAttribute?: string, uiHost?: string, uiPort?: string }, url: string | undefined) {
  ...
  await context._enableRecorder({
    ...
    uiPort: options.uiPort ? parseInt(options.uiPort, 10) : undefined,
  });
  await openPage(context, url);
  donePromise.resolve();
}
